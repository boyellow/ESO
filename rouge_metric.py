...
default_rougeL_scorer = rouge_scorer.RougeScorer(['rougeL'], use_stemmer=True)
default_rouge1_scorer = rouge_scorer.RougeScorer(['rouge1'], use_stemmer=True)
default_rouge2_scorer = rouge_scorer.RougeScorer(['rouge2'], use_stemmer=True)
...
def rouge(prediction, ground_truth, xlingual=False):
    scorer = default_rougeL_scorer
    scores = scorer.score(prediction=prediction, target=ground_truth)
    return scores["rougeL"].fmeasure


def rouge_1(prediction, ground_truth, xlingual=False):
    scorer = default_rouge1_scorer
    scores = scorer.score(prediction=prediction, target=ground_truth)
    return scores["rouge1"].fmeasure


def rouge_2(prediction, ground_truth, xlingual=False):
    scorer = default_rouge2_scorer
    scores = scorer.score(prediction=prediction, target=ground_truth)
...
def rougel_score(preds, targets):
...
def compute_all_metrics(predictions, references, xlingual=False):
    ...
    em, rouge1, rouge2, rougeL = 0, 0, 0, 0
    for pred, gold in zip(predictions, references): 
        ...
        # )
        rougeL_i = metric_max_over_ground_truths(
            rouge, prediction=pred, ground_truths=gold, xlingual=xlingual
        )
        rougeL += rougeL_i

        rouge1_i = metric_max_over_ground_truths(
            rouge_1, prediction=pred, ground_truths=gold, xlingual=xlingual
        )
        rouge1 += rouge1_i

        rouge2_i = metric_max_over_ground_truths(
            rouge_2, prediction=pred, ground_truths=gold, xlingual=xlingual
        )
        rouge2 += rouge2_i
    ...
    metrics = {"exact_match": em, "rouge1": rouge1, "rouge2": rouge2, "rougeL": rougeL}
