...
def add_data_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--max-full-length", type=int, default=512)
...
def add_gen_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--max-new-tokens", type=int, default=64)
    
    return parser


def add_norm_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group('normalization', 'normalization configurations')
    
    group.add_argument("--length-norm", action="store_true")
    return parser

def add_nes_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group('generation', 'generation configurations')
    group.add_argument("--online-gen", action="store_true")
    group.add_argument("--nes-sample", type=str, default='temp')
    group.add_argument("--nes-coef", type=float, default=0.1)
    group.add_argument("--nes-length-norm", action="store_true")
    group.add_argument("--reward-thres", type=float, default=None)
...
def get_args():
    ...
    parser = add_norm_args(parser)
    parser = add_gen_args(parser)
    parser = add_nes_args(parser)
    ...
    if args.type == "eval_main":
        ...
        args.save = save_path
    elif args.type == "lm_nes":
        save_path = os.path.join(
            args.save,
            (f"e{args.epochs}-bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}-N{args.n_gpu}-NN{args.n_nodes}-sample({args.nes_sample})-nes_coef{args.nes_coef}") )
        if args.length_norm:
            save_path = save_path + '-length_norm'
        if args.reward_thres is not None:
            save_path = save_path + f'-reward_thres{args.reward_thres}'
        args.save = save_path
    return args
