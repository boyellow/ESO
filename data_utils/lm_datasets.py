...
class LMTrainDataset(Dataset):
    def __init__(self, args, tokenizer, path, split, num, ratio, rng_sample: random.Random):
        ...
        self.max_full_length = args.max_full_length
        self.rng_sample = rng_sample
        self.lm_ctx = DistributedMMapIndexedDataset(path, f"{split}", get_rank(), get_world_size())

        if "xsum" or "HH" in args.data_dir:
            split_json = f"{split}.json"
        else:
            split_json = f"{split}.jsonl"
        if os.path.exists(os.path.join(path, split_json)):
            with open(os.path.join(path, split_json)) as f:
    ...
    def _process_lm(self, i, samp, model_data, no_model_data, gen_data):
        ...
        model_data["input_ids"][i][:input_len-1] = torch.tensor(input_ids[:-1], dtype=torch.long)  # input_ids shift
        ...
        no_model_data["label"][i][:input_len-1] = torch.tensor(input_ids[1:], dtype=torch.long) # label shift
        ...
        if prompt is not None:
            ...
            gen_data["target_ids"][i][-len(target):] = torch.tensor(target, dtype=torch.long)
    ...
    def collate(self, samples):
        ...
        gen_data = {
            ...
            "target_ids": torch.ones(bs, self.max_length, dtype=torch.long) * self.pad_id
        }
