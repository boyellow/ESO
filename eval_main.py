...
from rouge_metric import compute_metrics, compute_all_metrics
    ...
    for top_k in top_k_list:
        ...
        if get_rank() == 0:
            if args.eval_gen:
                ...
                res = compute_all_metrics(responses, references)
            
                # eval_dir = os.path.join(args.save)
                eval_dir = args.save
                print_rank(eval_dir)
                os.makedirs(eval_dir, exist_ok=True)
                with open(os.path.join(eval_dir, "answers_%s.jsonl"%top_k), "w") as f:
