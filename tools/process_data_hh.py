...
class Encoder(object): 
    ...
    def encode(self, line):
        ...
        if len(full_tokens) > self.args.max_full_length:
            return None, None, None, None, len(line)
...
def main():
    ...
    with open(os.path.join(args.data_dir, "sft_train.json")) as f:
        raw_data_train = f.readlines()

    with open(os.path.join(args.data_dir, "sft_test.json")) as f:
        raw_data_valid = f.readlines()
    ...
    for split in all_data:
        ...
        json_file = open(os.path.join(args.processed_data_dir, f"{split}.json"), "w")
